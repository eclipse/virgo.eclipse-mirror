...
public class AutoConfig implements DynamicDeployer, JndiConstants {
	...
	private static final String PROP_ADDITIONAL_REF_TYPES_TO_IGNORE = "openejb.ignore.additional.resource.refs";
	...
	private void resolvePersistenceRefs(AppModule appModule) {
		...
		for (ClientModule clientModule : appModule.getClientModules()) {
			URI moduleURI = URLs.uri(URLEncoder.encode(clientModule.getModuleId()));
			processPersistenceRefs(clientModule.getApplicationClient(), clientModule, persistenceUnits, moduleURI);
		}

		for (WebModule webModule : appModule.getWebModules()) {
			URI moduleURI = URLs.uri(URLEncoder.encode(webModule.getModuleId()));
	...
	private PersistenceUnit processPersistenceRef(AppModule appModule, LinkResolver<PersistenceUnit> persistenceUnits, PersistenceRef ref, URI moduleURI, String componentName,
	...
	/**
	 * Set destination, destinationType, clientId and subscriptionName in the
	...
	/**
	 * Set resource id in all message-destination-refs and MDBs that are using
	...
	private void resolveDestinationLinks(AppModule appModule) throws OpenEJBException {
		...
		// remember the type of each destination so we can use it to fillin MDBs
		// that don't declare destination type
		...
		// if MessageDestination does not have a mapped name assigned, give it
		// the destination from the MDB
		for (EjbModule ejbModule : appModule.getEjbModules()) {
			AssemblyDescriptor assembly = ejbModule.getEjbJar().getAssemblyDescriptor();
			if (assembly == null) {
				continue;
			...
			for (EnterpriseBean bean : ejbModule.getEjbJar().getEnterpriseBeans()) {
				...
				if (bean instanceof MessageDrivenBean) {
					...
					// resolve the destination... if we don't find one it is a
					// configuration bug
					...
					if (destinationId == null) {
						// if we don't have a mapped name use the destination of
						// the mdb
						Properties properties = mdb.getActivationConfig().toProperties();
		...
		// resolve all message destination refs with links and assign a ref id
		// to the reference
		for (EjbModule ejbModule : appModule.getEjbModules()) {
			AssemblyDescriptor assembly = ejbModule.getEjbJar().getAssemblyDescriptor();
			if (assembly == null) {
		...
		for (ClientModule clientModule : appModule.getClientModules()) {
			...
			for (MessageDestinationRef ref : clientModule.getApplicationClient().getMessageDestinationRef()) {
				String destinationId = resolveDestinationId(ref, moduleUri, destinationResolver, destinationTypes);
				if (destinationId != null) {
					// for client modules we put the destinationId in the mapped
					// name
		...
		for (WebModule webModule : appModule.getWebModules()) {
			...
			for (MessageDestinationRef ref : webModule.getWebApp().getMessageDestinationRef()) {
				String destinationId = resolveDestinationId(ref, moduleUri, destinationResolver, destinationTypes);
				if (destinationId != null) {
					// for web modules we put the destinationId in the mapped
					// name
					ref.setMappedName(destinationId);
				}
			}
		...
		// this time fill in the destination type (if not alreday specified)
		// with
		// the info from the destination (which got filled in from the
		// references)
		for (EjbModule ejbModule : appModule.getEjbModules()) {
			AssemblyDescriptor assembly = ejbModule.getEjbJar().getAssemblyDescriptor();
			if (assembly == null) {
			...
			URI moduleUri = URLs.uri(URLEncoder.encode(ejbModule.getModuleId()));
			OpenejbJar openejbJar = ejbModule.getOpenejbJar();
			...
			for (EnterpriseBean bean : ejbModule.getEjbJar().getEnterpriseBeans()) {
				...
				if (bean instanceof MessageDrivenBean) {
					...
					String link = mdb.getMessageDestinationLink();
					if (link != null && link.length() != 0) {
						// resolve the destination... if we don't find one it is
						// a configuration bug
						MessageDestination destination = destinationResolver.resolveLink(link, moduleUri);
						if (destination == null) {
							throw new OpenEJBException("Message destination " + link + " for message driven bean " + mdb.getEjbName() + " not found");
					...
					if (destinationType == null) {
						...
						// if destination name contains the string "queue" or
						// "topic" we use that
						Properties properties = mdb.getActivationConfig().toProperties();
						...
						logger.info("Auto-configuring a message driven bean " + ejbDeployment.getDeploymentId() + " destination " + properties.getProperty("destination") + " to be destinationType "
								+ destinationType);
					}
	...
	private String resolveDestinationId(MessageDestinationRef ref, URI moduleUri, LinkResolver<MessageDestination> destinationResolver, Map<MessageDestination, String> destinationTypes)
		...
		// resolve the destination... if we don't find one it is a configuration
		// bug
		MessageDestination destination = destinationResolver.resolveLink(link, moduleUri);
		if (destination == null) {
	...
	private void deploy(WebModule webModule, AppResources appResources) throws OpenEJBException {
		processJndiRefs(webModule.getModuleId(), webModule.getWebApp(), appResources, webModule.getClassLoader(), webModule);
	}
    ...
    private void processJndiRefs(String moduleId, JndiConsumer jndiConsumer, AppResources appResources, ClassLoader classLoader) throws OpenEJBException {
        processJndiRefs(moduleId, jndiConsumer, appResources, classLoader, null);
    }

    private void processJndiRefs(String moduleId, JndiConsumer jndiConsumer, AppResources appResources, ClassLoader classLoader, WebModule webModule) throws OpenEJBException {
		// Resource reference
		for (ResourceRef ref : jndiConsumer.getResourceRef()) {
			...
			// skip references such as URLs which are automatically handled by
			// the server
	...
	private void processApplicationResources(AppModule module) throws OpenEJBException {
		...
		for (Resource resource : resources) {
			...
			if (DataSource.class.getName().equals(resource.getType()) && resource.getProperties().containsKey(ORIGIN_FLAG)
		...
		resourceInfos.clear();
		// resources.clear(); // don't clear it since we want to keep this to be
		// able to undeploy resources with the app
	}
	...
	private void processResourceRef(ResourceRef ref, EjbDeployment ejbDeployment, AppResources appResources, EjbModule ejbModule) throws OpenEJBException {
		...
		// skip references such as URLs which are automatically handled by the
		// server
	...
	private void processResourceEnvRef(JndiReference ref, EjbDeployment ejbDeployment, AppResources appResources, ClassLoader classLoader) throws OpenEJBException {
		...
		// skip references such as SessionContext which are automatically
	...
	private void deploy(AppModule app, PersistenceModule persistenceModule) throws OpenEJBException {
		...
		for (PersistenceUnit unit : persistence.getPersistenceUnit()) {
			...
			// another try prefixing datasource name with app moduleid, case of
			// datasourcedefinition for instance
			...
			if (jtaDataSourceId == null && nonJtaDataSourceId == null) {
				...
				// we can just create the second resource using the first as a
				// template
				if (jtaDataSourceId != null || nonJtaDataSourceId != null) {
			...
			if (jtaDataSourceId == null && nonJtaDataSourceId == null) {
				...
				for (String id : ids) {
					...
					if (jtaDataSourceId == null) {
						// No jta managed data source found. Try finding a
						// non-jta managed
						required.clear();
						required.put("JtaManaged", "false");
					...
					if (jtaDataSourceId == null && nonJtaDataSourceId == null) {
						// Neither jta nor non-jta managed data sources were
						// found. try to find one with it unset
						required.clear();
						required.put("JtaManaged", NONE);
						jtaDataSourceId = findResourceId(id, "DataSource", required, null);
					}
			...
			// hibernate uses the fact that this ds is missing to get a non jta
			// em instead of a JTA one
				...
				if ("InitialSize".equalsIgnoreCase(key)) {
					try {
						...
						if (MAX_IMPLICIT_POOL_SIZE < value) {
							...
							logger.warning("Adjusting " + key + " to " + MAX_IMPLICIT_POOL_SIZE + " for " + copy.id + " DataSource to avoid too much network bandwidth usage."
	...
	private void checkUnitDataSourceRefs(PersistenceUnit unit) throws OpenEJBException {
		...
		// check that non-jta-data-source does NOT point to a JtaManaged=true
		// datasource
		...
		if (invalidNonJta != null) {
			throw new OpenEJBException("PeristenceUnit " + unit.getName() + " <non-jta-data-source> points to a jta managed Resource.  Update Resource \"" + invalidNonJta
		...
		// check that jta-data-source does NOT point to a JtaManaged=false
		// datasource
		...
		if (invalidJta != null) {
			throw new OpenEJBException("PeristenceUnit " + unit.getName() + " <jta-data-source> points to a non jta managed Resource.  Update Resource \"" + invalidJta
	...
	private String getResourceId(String beanName, String resourceId, String type, AppResources appResources, String moduleId) throws OpenEJBException {
		return getResourceId(beanName, resourceId, type, null, appResources, moduleId);
	}
	...
	private String getResourceId(String beanName, String resourceId, String type, Properties required, AppResources appResources) throws OpenEJBException {
		return getResourceId(beanName, resourceId, type, required, appResources, null);
	}

	private String getResourceId(String beanName, String resourceId, String type, Properties required, AppResources appResources, String moduleId) throws OpenEJBException {
		...
		// skip references such as URL which are automatically handled by the
		// server
		if (type != null && ignoredReferenceTypes.contains(type)) {
			return null;
		}

		// check for existing resource with specified resourceId and type and
		// properties
		String id = findResourceId(resourceId, type, required, appResources, moduleId);
		if (id != null)
			return id;

		// expand search to any type -- may be asking for a reference to a
		// sub-type
		id = findResourceId(resourceId, null, required, appResources, moduleId);
		if (id != null)
			return id;

		// throw an exception or log an error
		String shortName = toShortName(resourceId);
		String message = "No existing resource found while attempting to Auto-link unmapped resource-ref '" + resourceId + "' of type '" + type + "' for '" + beanName + "'.  Looked for Resource(id="
				+ resourceId + ") and Resource(id=" + shortName + ")";
		if (!autoCreateResources) {
	...
	private String findResourceId(String resourceId, String type, Properties required, AppResources appResources) {
		return findResourceId(resourceId, type, required, appResources, null);
	}

	private String findResourceId(String resourceId, String type, Properties required, AppResources appResources, String moduleId) {
		if (resourceId == null)
			return null;
		...
		if (moduleId != null) {
			for (String id : resourceIds) {
				int index = id.indexOf(moduleId);
				String tempId = id;
				if (index != -1) {
				    tempId = id.substring(index + 1 + moduleId.length());
				}
				if (tempId.equalsIgnoreCase(resourceId))
					return id;
			}
		}
	...
	private String toShortName(String resourceId) {
		// check for an existing resource using the short name (everything ever
		// the final '/')
	...
	private String getResourceEnvId(String beanName, String resourceId, String type, AppResources appResources) throws OpenEJBException {
		...
		// throw an exception or log an error
		String message = "No existing resource found while attempting to Auto-link unmapped resource-env-ref '" + resourceId + "' of type '" + type + "' for '" + beanName
				+ "'.  Looked for Resource(id=" + resourceId + ")";
		if (!autoCreateResources) {
			throw new OpenEJBException(message);
		}
		logger.debug(message);
		...
		if (providerId == null) {
			// if there are any existing resources of the desired type, use the
			// first one
	...
	private static class AppResources {
		...
		public AppResources(AppModule appModule) {
			...
			//
			// DEVELOPERS NOTE: if you change the id generation code here, you
			// must change
			// the id generation code in
			...
			// final String id = module.getUniqueId() + '/' +
			// ds.getName().replace("java:", "");
			// if (resourceIdsByType.get("javax.sql.DataSource") == null) {
			// resourceIdsByType.put("javax.sql.DataSource", new
			// ArrayList<String>());
			// }
	...
	private static void processAdditionalRefsToIgnore() {		
		String additionalRefTypesToIgnore = System.getProperty(PROP_ADDITIONAL_REF_TYPES_TO_IGNORE);
		logger.debug("Processing Aditiona ref types to ignore " + additionalRefTypesToIgnore);
		if(additionalRefTypesToIgnore != null) {
			String [] types = additionalRefTypesToIgnore.split(",");
			for(String type: types) {
				ignoredReferenceTypes.add(type.trim());
			}
		}
	}
}
