 ...
 * {@link org.apache.webbeans.spi.ScannerService} at the scanner phase.
 */
		...
		if (beans != null && beans.size() > 0) {
			...
			for (Bean<?> bean : beans) {
				...
				if (bean instanceof InjectionTargetBean) {
					...
					if (!(bean instanceof Decorator) && !(bean instanceof javax.enterprise.inject.spi.Interceptor) && !(bean instanceof NewBean)) {
				...
				if (injectionPoints != null) {
					for (InjectionPoint injectionPoint : injectionPoints) {
						if (!injectionPoint.isDelegate()) {
							...
							if (!bean.getBeanClass().isAnnotationPresent(javax.decorator.Decorator.class) && !webBeansContext.getBeanManagerImpl().containsCustomDecoratorClass(bean.getBeanClass())) {
				...
				for (Class<?> specialClass : beanClasses) {
					if (AnnotationUtil.hasClassAnnotation(specialClass, Specializes.class)) {
						...
						if (superClass.equals(Object.class)) {
							throw new WebBeansConfigurationException("Specialized class [" + specialClass.getName() + "] must extend another class.");
						}
		...
		if (webBeansContext.getManagedBeanConfigurator().isManagedBean(clazz)) {
			...
			if (processInjectionTarget != null) {
				if (processInjectionTargetEvent != null) {
					...
					final InjectionTarget updatedInjectionTarget = webBeansContext.getWebBeansUtil().fireProcessInjectionTargetEvent(processInjectionTargetEvent).getInjectionTarget();
